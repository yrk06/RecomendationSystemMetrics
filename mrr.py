...
def main(args):
    ...
    pandasExport(results,args.a+'-'+args.b+'.mrr')
...
def calculateSingleThread(arqs):
    ...
    for arq in arqs:
        ...
        calculateMRRArchive(arq,q)
        q += 1

def calculateMRRArchive(arq,idx):
...
def calculate(user):
    itemList = generateList(user)
    for idx in range(len(itemList)):
        item = itemList[idx]
        if item[1] == 1:
            ##Achamos o primeiro item relevante
            return 1/(idx+1)
    return 0
